    ...
    public void alg(ArrayList<Procesor> list, int maxRoznicaMiedzyProcesami, int maxMoment, int liczbaLosowan, int max, int min) {
        ...
        while (obecnyCzas < maxMoment + maxRoznicaMiedzyProcesami) {
            for (Procesor x : list) {
                ...
                for(Procesor xxx: list) {
                    if (xxx.getObciazenie() < min) {
                        for (Procesor xx : list) {
                            liczbaPorownan++;
                            if (xx.getObciazenie() > progPrzejecia && xx != xxx) {
                                liczbaMigracji++;
                                Zadanie temp = xx.getAktywneZadania().get(0);
                                xx.obnizObciazenie(temp.getZapelnienie());
                                xx.getAktywneZadania().remove(0);
                                xxx.getAktywneZadania().add(0, temp);
                                xxx.setObciazenie(temp.getZapelnienie());
                                break;
                            }
                        }
                    }
        ...
        System.out.println("liczba porownan 3: " + liczbaPorownan);
        System.out.println("liczba migracji 3: " + liczbaMigracji);
